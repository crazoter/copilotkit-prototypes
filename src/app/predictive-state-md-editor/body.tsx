...
export default function PredictiveStateUpdates({ params }: PredictiveStateUpdatesProps) {
  ...
  // Initialize mobile chat with chatDefaultOpen only once
  useEffect(() => {
    if (isMobile && chatDefaultOpen) {
      setIsChatOpen(true);
    }
  ...
  return (
    <CopilotKit
      runtimeUrl={getAgentRuntimeUrl("predictive_state_md_editor")}
      showDevConsole={false}
      // agent lock to the relevant agent
      agent="predictive_state_md_editor"
    >
      <div
...
const DocumentEditor = () => {
  ...
  const {
    ...
    name: "predictive_state_md_editor",
    initialState: {
      document: "",
    },
  });
  ...
  useEffect(() => {
    ...
    // Guard: Check if placeholder visibility needs to change
    const shouldShowPlaceholder = text.length === 0;
    setPlaceholderVisible((prev) => (prev !== shouldShowPlaceholder ? shouldShowPlaceholder : prev));

    if (!isLoading) {
      // Guard: Only update if the text has actually changed
  ...
  }, [editorState, isLoading, currentDocument]);
